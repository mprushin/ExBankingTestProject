defmodule ExBankingTest do
  ...
  test "user doesn't exist" do
    assert {:error, :user_does_not_exist} = ExBanking.get_balance("user which is doesn't exist", "rur")
  end
  ...
  test "sender/receiver does not exist" do
    ...
    assert {:error, :sender_does_not_exist} = ExBanking.send("user which is doesn't exist", "test5", 10, "rur")
    assert {:error, :receiver_does_not_exist} = ExBanking.send("test5", "user which is doesn't exist", 10, "rur")
  end
